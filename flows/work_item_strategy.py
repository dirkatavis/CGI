...
class WorkItemHandler(ABC):
    ...
    def create_work_item(self, config: WorkItemConfig) -> Dict[str, Any]:
        """
        Main entry point for creating work items using this handler.
        
        Template method that orchestrates the common workflow:
        1. Click Add Work Item button
        2. Check for existing complaints
        3. Either associate existing or create new complaint
        4. Complete work item specific steps
        """
    ...
    def _click_add_work_item_button(self, config: WorkItemConfig) -> bool:
        """Click the Add Work Item button. Common implementation for all strategies."""
    ...
    def _handle_complaint_flow(self, config: WorkItemConfig) -> Dict[str, Any]:
        """Handle the complaint association/creation flow."""
...
class GlassWorkItemHandler(WorkItemHandler):
    ...
    def create_new_complaint(self, config: WorkItemConfig) -> Dict[str, Any]:
        """
        Create new glass complaint.
        
        Implements the scenario:
        Click: Add Work Item Btn; Prompt- Is vehicle drivable?(click No btn); 
        select issue type btn(click "Glass Damage" btn); 
        Damage Type(click btn based on csv(Crack, chip, Side/rear)); Click Submit Complaint btn;
        """
    ...
    def handle_existing_complaint(self, config: WorkItemConfig, complaint_element) -> Dict[str, Any]:
        """
        Handle existing glass complaint scenarios.
        
        Two scenarios:
        1. Glass repair with existing complaint: Select existing complaint; Next; Mileage-Next; OpCode; Done
        2. Glass replacement with existing complaint: Select existing complaint; Select workitem type (Replace); Mileage-Next; OpCode; Done
        """
