...
def get_driver_version(driver_path: str) -> str:
    ...
    if not os.path.exists(driver_path):
        log.error("[DRIVER] Driver binary not found at %s", driver_path)
        return "unknown"
...
def _major_version(version: str) -> str | None:
    """Return the major version component for a semantic version string."""
    match = re.match(r"(\d+)\.", version)
    return match.group(1) if match else None


def _build_edge_options() -> webdriver.EdgeOptions:
    """Build the Edge options shared by bundled and auto-managed drivers."""
...
def _launch_edge(options: webdriver.EdgeOptions, service: Service | None = None):
    """Launch Edge with an optional explicit driver service."""
    if service is None:
...
def create_driver():
    ...
    log.info("[DRIVER] Detected Browser=%s, bundled Driver=%s", browser_ver, driver_ver)
    options = _build_edge_options()

    # Always prefer Selenium Manager — it downloads a driver that matches the
    # installed Edge version automatically. The bundled msedgedriver.exe is
    # kept only as a last-resort fallback for air-gapped environments.
    log.info("[DRIVER] Launching Edge via Selenium Manager (Browser %s)", browser_ver)
    try:
        _STATE["driver"] = _launch_edge(options)
        return _STATE["driver"]
    except (WebDriverException, OSError) as exc:  # pylint: disable=broad-exception-caught
        log.warning(
